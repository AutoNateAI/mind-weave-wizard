-- Update existing prompts and add missing ones for edge functions
UPDATE ai_prompts SET 
  prompt_template = 'You are an expert course designer creating the "AutoNateAI: Thinking Wizard Course" - a 10-session mental mastery journey focused on graph theory and thinking models.
...
Make sure each session has exactly 3 lectures, each 5-7 minutes long. Focus on engaging, practical content that builds progressively.',
  variables = '[{"name": "course_description", "type": "text", "description": "Description of the course to be created"}, {"name": "chat_history", "type": "text", "description": "JSON string of chat history for context"}]',
  feature_page = 'course_planning_tab'
WHERE prompt_name = 'course_planning_system';

-- Insert new prompts that don't exist yet
INSERT INTO ai_prompts (prompt_name, prompt_category, prompt_template, prompt_description, variables, feature_page) 
SELECT * FROM (VALUES
...
) AS new_prompts(prompt_name, prompt_category, prompt_template, prompt_description, variables, feature_page)
WHERE NOT EXISTS (
  SELECT 1 FROM ai_prompts WHERE ai_prompts.prompt_name = new_prompts.prompt_name
);